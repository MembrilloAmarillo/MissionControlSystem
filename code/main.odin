  ...
  if app_state.menu_db.current_tab != &UI_NilBox || ok {
   ...
   case "TM Parameters": {
    ...
    for system := system_node; system != nil && !limit_on_screen; system = system.next {
     for parameter in xtce.GetSpaceSystemParameterSetTypes(system.element) {
       ...
       else if p == &content_panel
       {
        switch app_state.SHOW_FLAGS {
         ...
         case APP_SHOW_FLAGS.SHOW_TC:
         {
          ...
          set_box_preferred_size({0.5 * p.rect.size.x, 35})
          if begin( "TC Center#tc_center%p", pointer = cast(^byte)p )
          {
            set_layout_next_column(0)
            set_layout_next_row(0)
            ui_vspacer(5.)
            input_field("CCSDS Packet ID"); ui_vspacer(5.)
            set_layout_next_row(1)
            input_field("CCSDS Packet Service Type"); ui_vspacer(5.)
            set_layout_next_row(2)
            input_field("CCSDS Packet Subservice Type"); ui_vspacer(5.)
            set_layout_next_row(3)
            input_field("CCSDS Packet Data")
          }
         }
