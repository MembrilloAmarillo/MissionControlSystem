...
debug_time_add_scope :: proc( name : string, allocator := context.allocator ) -> (scope_name : string, tick : time.Tick)
{
	return _debug_time_add_scope( name, allocator )
}

_debug_time_add_scope :: proc( name : string, allocator := context.allocator ) -> (scope_name : string, tick : time.Tick)
{
...
CHECK_MEM_ERROR :: proc ( error : vmem.Allocator_Error ) {
	...
	{
		case .None                 : break;
		case .Out_Of_Memory        : fallthrough
		case .Invalid_Pointer      : fallthrough
		case .Invalid_Argument     : fallthrough
		case .Mode_Not_Implemented : fallthrough
		case : fmt.println("[ERROR] Allocation error ", error);
...
mouse_button_callback :: proc "c" (window: glfw.WindowHandle, button, action, mods : i32) {
	...
	if( button == glfw.MOUSE_BUTTON_LEFT && action == glfw.PRESS ) {
		...
		append(&app.va_OsInput, os_input{os_input_types{ .LEFT_CLICK }, {cast(f32)xpos, cast(f32)ypos}, 0, {0, 0}});
	}
	else if( button == glfw.MOUSE_BUTTON_LEFT && action == glfw.RELEASE ) {
		xpos, ypos := glfw.GetCursorPos(app.va_Window.w_window);
		append(&app.va_OsInput, os_input{os_input_types{ .LEFT_CLICK_RELEASE }, {cast(f32)xpos, cast(f32)ypos}, 0, {0, 0}});
	}
}
...
debugCallback :: proc "stdcall" ( messageSeverity : vk.DebugUtilsMessageSeverityFlagsEXT, messageType: vk.DebugUtilsMessageTypeFlagsEXT, pCallbackData : ^vk.DebugUtilsMessengerCallbackDataEXT, pUserData : rawptr ) -> b32
{
	context = runtime.default_context();
...
find_queue_families :: proc( v_interface : ^VulkanIface, device : vk.PhysicalDevice ) -> QueueFamilyIndices
{
	...
	for &family in queue_families {
		...
		present_support :b32;
		if vk.GetPhysicalDeviceSurfaceSupportKHR( device, i, v_interface.va_Window.w_surface, &present_support ) != .SUCCESS {
			panic("[ERROR] GetPhysicalDeviceSurfaceSupportKHR was not succesful")
		}
		if present_support {
			indices.qfi_Presentation = i;
		}
		if indices.qfi_GraphicsAndCompute != bits.U32_MAX && indices.qfi_Presentation != bits.U32_MAX {
...
check_device_extension_support :: proc( v_interface : ^VulkanIface, device : vk.PhysicalDevice ) -> bool
{
	extension_count : u32;
	vk.EnumerateDeviceExtensionProperties( device, nil, &extension_count, nil );

	available_extension := make([]vk.ExtensionProperties, extension_count, v_interface.ArenaAllocator );

	vk.EnumerateDeviceExtensionProperties( device, nil, &extension_count, raw_data(available_extension));
...
query_swapchain_support :: proc( v_interface : ^VulkanIface, device : vk.PhysicalDevice, allocator := context.allocator ) -> SwapchainSupportDetails
{
	details : SwapchainSupportDetails;
	vk.GetPhysicalDeviceSurfaceCapabilitiesKHR( device, v_interface.va_Window.w_surface, &details.scsd_Capabilities );
	vk.GetPhysicalDeviceSurfaceFormatsKHR( device, v_interface.va_Window.w_surface, &details.scsd_FormatCount, nil);
	...
	if details.scsd_FormatCount != 0 {
		...
		vk.GetPhysicalDeviceSurfaceFormatsKHR( device, v_interface.va_Window.w_surface, &details.scsd_FormatCount, raw_data(details.scsd_Formats) );
	}

	vk.GetPhysicalDeviceSurfacePresentModesKHR(device, v_interface.va_Window.w_surface, &details.scsd_PresentModeCount, nil );

	details.scsd_PresentModes = nil;
	if details.scsd_PresentModeCount != 0 {
        details.scsd_PresentModes = make( []vk.PresentModeKHR, details.scsd_PresentModeCount, allocator );
        vk.GetPhysicalDeviceSurfacePresentModesKHR(device, v_interface.va_Window.w_surface, &details.scsd_PresentModeCount, raw_data(details.scsd_PresentModes));
    }
...
is_suitable_device :: proc( v_interface : ^VulkanIface, device : vk.PhysicalDevice ) -> bool
{
	...
	if extensions_supported {
		...
		swapchain_adequate = swapchain_support.scsd_Formats != nil && swapchain_support.scsd_PresentModes != nil;
	...
	return (indices.qfi_GraphicsAndCompute != bits.U32_MAX && indices.qfi_Presentation != bits.U32_MAX && extensions_supported && swapchain_adequate);
}
...
create_image :: proc ( vi : ^VulkanIface, img : ^VulkanImage )
{
	...
	allocInfo.memoryTypeIndex = find_memory_type( vi.va_Device.d_PhysicalDevice, memRequirements.memoryTypeBits, img.vi_Properties );

    if vk.AllocateMemory(vi.va_Device.d_LogicalDevice, &allocInfo, nil, &img.vi_DeviceMemory) != .SUCCESS {
...
find_depth_format :: proc( vi: ^VulkanIface, depthFormats : []vk.Format ) -> Maybe(vk.Format)
{
	...
	for format in depthFormats {
		...
		if tiling == vk.ImageTiling.LINEAR && .DEPTH_STENCIL_ATTACHMENT in props.linearTilingFeatures {
			depthFormat = format;
			break;
		}
		else if tiling == vk.ImageTiling.OPTIMAL && .DEPTH_STENCIL_ATTACHMENT in props.optimalTilingFeatures {
			depthFormat = format;
			break;
		}
	}
...
create_depth_resources :: proc( vi : ^VulkanIface)
{
	...
	if vk.CreateImageView( vi.va_Device.d_LogicalDevice, &create_info, nil, &vi.va_DepthImage.vi_ImageView ) != .SUCCESS {
...
create_image_views :: proc( vi : ^VulkanIface )
{
	...
	for &image in vi.va_SwapChain.sc_Images {
		...
		if vk.CreateImageView( vi.va_Device.d_LogicalDevice, &create_info, nil, &vi.va_SwapChain.sc_ImageViews[i] ) != .SUCCESS {
			panic("[ERROR] Could not create image view");
		}
		i = i + 1;
	}
...
create_swap_chain :: proc( vi : ^VulkanIface )
{
	...
	if support.scsd_Capabilities.maxImageCount > 0 && image_count > support.scsd_Capabilities.maxImageCount {
  ...
  // present_mode = .IMMEDIATE
	...
	if vk.CreateSwapchainKHR( vi.va_Device.d_LogicalDevice, &sc_create_info, nil, &vi.va_SwapChain.sc_SwapChainHandle ) != .SUCCESS {
...
create_render_pass :: proc( vi : ^VulkanIface )
{
	...
	if vk.CreateRenderPass( vi.va_Device.d_LogicalDevice, &render_pass_info, nil, &vi.va_RenderPass[render_pass_idx] ) != .SUCCESS {
...
add_descriptor_set_layout :: proc( vi : ^VulkanIface, layouts : []vk.DescriptorType )
{
    ...
    if (vk.CreateDescriptorSetLayout( vi.va_Device.d_LogicalDevice, &layoutInfo, nil, &vi.va_Descriptors[descriptor_idx].d_DescriptorSetLayout) != .SUCCESS) {
...
add_pipeline :: proc ( vi : ^VulkanIface, pipeline : ^Pipeline, vert_data : []u8, frag_data : []u8, type : PipelineType )
{
  ...
  vertShaderStageInfo := vk.PipelineShaderStageCreateInfo{
	...
	#partial switch( type ) {
		case .GRAPHICS2D:
		{
            ...
            attributeDescriptions[9].offset = 4 * size_of(glsl.vec4) +  4 * size_of(glsl.vec2) + size_of(f32);

            attributeDescriptions[10].binding = 0;
            attributeDescriptions[10].location = 10;
            attributeDescriptions[10].format = .R32_SFLOAT;
            attributeDescriptions[10].offset = 4 * size_of(glsl.vec4) +  4 * size_of(glsl.vec2) + 2 * size_of(f32);
    ...
    if (vk.CreatePipelineLayout(vi.va_Device.d_LogicalDevice, &pipelineLayoutInfo, nil, &(pipeline.p_PipelineLayout)) != .SUCCESS) {
    ...
    if (vk.CreateGraphicsPipelines(vi.va_Device.d_LogicalDevice, 0, 1, &pipelineInfo, nil, &(pipeline.p_Pipeline)) != .SUCCESS) {
...
create_frame_buffer :: proc( vi : ^VulkanIface )
{
	if vi.va_SwapChain.sc_Framebuffers == nil {
		vi.va_SwapChain.sc_Framebuffers  = make( [dynamic]vk.Framebuffer, len(vi.va_SwapChain.sc_ImageViews), vi.ArenaAllocator );
	}
	else {
    ...
    {
        attachments := [?]vk.ImageView{ vi.va_SwapChain.sc_ImageViews[i], vi.va_DepthImage.vi_ImageView };
        ...
        if (vk.CreateFramebuffer(vi.va_Device.d_LogicalDevice, &framebufferInfo, nil, &(vi.va_SwapChain.sc_Framebuffers[i])) != .SUCCESS) {
...
create_buffer :: proc( vi : ^VulkanIface, buffer : ^VulkanBuffer, loc := #caller_location)
{
    ...
    if (vk.CreateBuffer(vi.va_Device.d_LogicalDevice, &bufferInfo, nil, &buffer.vb_Buffer) != .SUCCESS) {
        panic( "[ERROR] Failed to create buffer\n");
    }

    memRequirements : vk.MemoryRequirements;
    vk.GetBufferMemoryRequirements(vi.va_Device.d_LogicalDevice, buffer.vb_Buffer, &memRequirements);

    allocInfo : vk.MemoryAllocateInfo;
    allocInfo.sType = .MEMORY_ALLOCATE_INFO;
    allocInfo.allocationSize = memRequirements.size;
    allocInfo.memoryTypeIndex = find_memory_type(vi.va_Device.d_PhysicalDevice, memRequirements.memoryTypeBits, buffer.vb_Properties);

    if (vk.AllocateMemory(vi.va_Device.d_LogicalDevice, &allocInfo, nil, &buffer.vb_DeviceMemory) != .SUCCESS)
    {
        fmt.println(loc)
    ...
    fmt.println("[INFO] Batch size       :", size_of(Vertex2D) * len(vertices))
        ...
        if v_buffer.vb_toDestroy {
          ...
          fmt.println("[INFO] Recreating buffer after destruction")
        	create_vertex_2D_buffer( vi, &vt_buffer, &id_buffer, vertices, indices, auto_cast v_buffer.vb_Size );
          ...
          fmt.println("[INFO] Resizing buffer to size:", cast(int)buffer_size + MAX_BUFFER_2D_SIZE_BYTES )
          ...
          vt_buffer : VulkanBuffer;
          id_buffer : VulkanBuffer;
          create_vertex_2D_buffer( vi, v_buffer, i_buffer, vertices, indices, cast(int)buffer_size + MAX_BUFFER_2D_SIZE_BYTES );
         ...
         staging_buffer.vb_UsageFlags = vk.BufferUsageFlags{ .TRANSFER_SRC };
    	    ...
    	    data : rawptr;
    	    vk.MapMemory( vi.va_Device.d_LogicalDevice, staging_buffer.vb_DeviceMemory, 0, buffer_size, memory_map_flag, &data);
          ...
          v_buffer.vb_Size = buffer_size
    	    ...
    	    vk.MapMemory( vi.va_Device.d_LogicalDevice, staging_buffer.vb_DeviceMemory, 0, idx_buffer_size, memory_map_flag, &data);
...
create_vertex_2D_buffer :: proc( vi : ^VulkanIface, v_buffer : ^VulkanBuffer, i_buffer : ^VulkanBuffer, vertices : [dynamic]Vertex2D, indices : [dynamic]u32, size := MAX_BUFFER_2D_SIZE_BYTES, loc := #caller_location )
{
 debug_time_add_scope( "vk buffer creation", vi.ArenaAllocator );
  ...
  data : rawptr;
  vk.MapMemory( vi.va_Device.d_LogicalDevice, staging_buffer.vb_DeviceMemory, 0, auto_cast vertex_size, memory_map_flag, &data);
  ...
  create_buffer( vi, &staging_buffer );
     ...
     memory_map_flag: vk.MemoryMapFlags;
  data : rawptr;
  vk.MapMemory( vi.va_Device.d_LogicalDevice, staging_buffer.vb_DeviceMemory, 0, auto_cast idx_size, memory_map_flag, &data);
...
transition_image_layout :: proc( vi : ^VulkanIface, old_img, new_img : ^VulkanImage )
{
    ...
    else if (old_img.vi_Layout == .TRANSFER_DST_OPTIMAL && new_img.vi_Layout == .SHADER_READ_ONLY_OPTIMAL)
    {
...
recreate_swapchain :: proc( vi : ^VulkanIface )
{
  ...
  for i in 0..<MAX_FRAMES_IN_FLIGHT {
    vk.DestroySemaphore( vi.va_Device.d_LogicalDevice, vi.va_Semaphores.s_RenderFinished[i], nil)
    vk.DestroySemaphore( vi.va_Device.d_LogicalDevice, vi.va_Semaphores.s_ImageAvailable[i], nil)
...
init_glfw :: proc( vi : ^VulkanIface )
{
  ...
  glfw.WindowHint( glfw.AUTO_ICONIFY, false );
	...
	vi.va_Window.w_window = glfw.CreateWindow(vi.va_Window.w_width, vi.va_Window.w_height, "OrbitMCS", nil, nil)
 ...
 monitors_handle := glfw.GetMonitors()
	vi.va_Window.scaling_factor.x, vi.va_Window.scaling_factor.y = glfw.GetMonitorContentScale( monitors_handle[0] )
...
render_pass :: proc( vi : ^VulkanIface, command_buffer : vk.CommandBuffer, pipeline : Pipeline, descriptor : ^DescriptorSet )
{
	//tracy.ZoneS(depth = 10)
...
update_uniform_buffer :: proc( vi : ^VulkanIface )
{
 //vi.va_CurrentUB_UI.time        = cast(f32)vi.glfw_timer
	...
	mem.copy(vi.va_UniformBufferUI[vi.va_CurrentFrame].vb_MappedMemory, &vi.va_CurrentUB_UI, size_of(UniformBufferUI));
}
...
add_batch_to_destroy :: proc( vi : ^VulkanIface, frame : u32)
{
 ...
 n_batches_erase : u32 = 0
  ...
  if vi.va_2DBufferBatch.bb_VertexBuffer[i].vb_toDestroy {
    n_batches_erase += 1
    append(&vi.va_2DBufferBatchDestroy[frame].bb_VertexBuffer, vi.va_2DBufferBatch.bb_VertexBuffer[i]);
  }
  ...
  if vi.va_2DBufferBatch.bb_IndexBuffer[i].vb_toDestroy {
    append(&vi.va_2DBufferBatchDestroy[frame].bb_IndexBuffer, vi.va_2DBufferBatch.bb_IndexBuffer[i]);
  }
	}
...
create_semaphores :: proc( vi : ^VulkanIface )
{
  ...
  for i in 0..<MAX_FRAMES_IN_FLIGHT
  {
      if (vk.CreateSemaphore(vi.va_Device.d_LogicalDevice, &semaphoreInfo, nil, &(vi.va_Semaphores.s_ImageAvailable[i])) != .SUCCESS ||
          vk.CreateSemaphore(vi.va_Device.d_LogicalDevice, &semaphoreInfo, nil, &(vi.va_Semaphores.s_RenderFinished[i])) != .SUCCESS ||
          vk.CreateFence(vi.va_Device.d_LogicalDevice, &fenceInfo, nil, &(vi.va_Semaphores.s_InFlight[i])) != .SUCCESS)
      {
...
draw_frame :: proc( vi : ^VulkanIface )
{
	...
	vk.ResetFences( vi.va_Device.d_LogicalDevice, 1, &vi.va_Semaphores.s_InFlight[vi.va_CurrentFrame]);
	...
	add_batch_to_destroy(vi, vi.va_CurrentFrame);
	if vk.QueueSubmit( vi.va_Device.d_GraphicsQueue, 1, &submit_info, vi.va_Semaphores.s_InFlight[vi.va_CurrentFrame]) != .SUCCESS {
...
init_vulkan :: proc( v_interface : ^VulkanIface ) {
	...
	if( glfw.CreateWindowSurface( v_interface.va_Instance, v_interface.va_Window.w_window, nil, &v_interface.va_Window.w_surface ) != .SUCCESS )
	{
	...
	when ODIN_DEBUG {
		if( vk.CreateDebugUtilsMessengerEXT(
				...
				&debugCreateInfo,
				nil,
				&v_interface.va_DebugMessenger ) != .SUCCESS )
		{
 ...
 device_create_info.ppEnabledExtensionNames = &DEVICE_EXTENSIONS[0];
	...
	if vk.CreateDevice( v_interface.va_Device.d_PhysicalDevice, &device_create_info, nil, &v_interface.va_Device.d_LogicalDevice ) != .SUCCESS {
		panic( "[ERROR] Could not create logical device");
	}

	vk.GetDeviceQueue( v_interface.va_Device.d_LogicalDevice, queue_families.qfi_GraphicsAndCompute, 0, &v_interface.va_Device.d_GraphicsQueue);
	vk.GetDeviceQueue( v_interface.va_Device.d_LogicalDevice, queue_families.qfi_GraphicsAndCompute, 0, &v_interface.va_Device.d_ComputeQueue);
 	vk.GetDeviceQueue( v_interface.va_Device.d_LogicalDevice, queue_families.qfi_Presentation,       0, &v_interface.va_Device.d_PresentationQueue);
	...
	add_descriptor_set_layout( v_interface, {.UNIFORM_BUFFER, .UNIFORM_BUFFER, .COMBINED_IMAGE_SAMPLER} );
	...
	add_pipeline( v_interface, &v_interface.va_Pipelines[0], vertex2D_data, fragment2D_data, PipelineType.GRAPHICS2D );

	add_pipeline( v_interface, &v_interface.va_Pipelines[1], vertex3D_data, fragment3D_data, PipelineType.GRAPHICS3D );

	add_pipeline( v_interface, &v_interface.va_Pipelines[2], compute_data, nil, PipelineType.COMPUTE );
    ...
    if (vk.CreateCommandPool(v_interface.va_Device.d_LogicalDevice, &poolInfo, nil, &(v_interface.va_CommandPool)) != .SUCCESS) {
  ...
  batch : Batch2D;
  append(&batch.vertices, Vertex2D{
  	{0.0, 0.0},
    {200.0, 200.0},
    {0.2, 0.2, 0.2, 1.0},
    {0.1, 0.1, 0.1, 1.0},
    ...
    for i in 0..<MAX_FRAMES_IN_FLIGHT
    {
     ...
     v_interface.va_UniformBufferUI[i].vb_Properties = vk.MemoryPropertyFlags{ .HOST_VISIBLE, .HOST_COHERENT };
     create_buffer(v_interface, &v_interface.va_UniformBufferUI[i] );
     memory_map_flag : vk.MemoryMapFlags;
     vk.MapMemory(v_interface.va_Device.d_LogicalDevice,
      v_interface.va_UniformBufferUI[i].vb_DeviceMemory,
      0, size_of(UniformBufferUI), memory_map_flag,
     ...
     v_interface.va_UniformBuffer3D[i].vb_Properties = vk.MemoryPropertyFlags{ .HOST_VISIBLE, .HOST_COHERENT };
     create_buffer(v_interface, &v_interface.va_UniformBuffer3D[i] );

     vk.MapMemory(v_interface.va_Device.d_LogicalDevice,
      v_interface.va_UniformBuffer3D[i].vb_DeviceMemory,
      0, size_of(UniformBuffer3D), memory_map_flag,
    ...
    // -------------- Creates descriptor pool -------------------- //
    {
        ...
        if (vk.CreateDescriptorPool(v_interface.va_Device.d_LogicalDevice, &poolInfo, nil, &v_interface.va_DescriptorPool) != .SUCCESS)
        {
    ...
    // -------------- Creates command buffers -------------------- //
    {
        ...
        if (vk.AllocateCommandBuffers(v_interface.va_Device.d_LogicalDevice, &allocInfo, raw_data(v_interface.va_CommandBuffers[:])) != .SUCCESS)
        {
