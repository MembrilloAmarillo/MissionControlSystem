...
traverse_complex_type_choice_selection :: proc(
	...
	for node_it = auto_cast choice_root.next;
	    node_it.right != nil;
	    node_it = auto_cast node_it.right {}
			...
			case xml.Element_ID:
				{
					...
					{
						if child_el.ident == "element" &&
						   it.ident == "choice" &&
						   len(child_el.attribs) > 0 {

							node: ^node_tree(xml.Attributes) = new(
								node_tree(xml.Attributes),
								allocator,
							)
							...
							push_stack(&q_node_it, node_it)
							push_stack(&q_node, child_el)
							if it.ident == "restriction" ||
							   it.ident == "enumeration" ||
							   it.ident == "simpleType" {
								append(
									&node_it.element,
									xml.Attribute{key = child_el.ident, val = "embedded"},
								)
...
hash_schema :: proc(schema: ^xsd_schema, allocator := context.allocator) -> schema_type_def {
	...
	for xsd_element_stack.push_count > 0 {
		...
		if (element.ident == "complexType" || element.ident == "simpleType") &&
		   ...
		   len(element.attribs) > 0 { 	// this last comprobation is to know that is not a nested type
				...
				case complex_content:
					{
						...
						if content.nested_content != nil {
							...
							append(
								...
								xml.Attribute {
									key = "type",
									val = strings.concatenate({"xtce:", type_name}),
								},
							)
						}
					}
				}
				my_hash.insert_table(
					&schema.xsd_hash,
					current_type_def.type_name,
					current_type_def,
				)
		...
		} else if (element.ident == "simpleType") &&
		   len(element.attribs) == 0 &&
		   len(current_type_def.type_name) > 0 {
		...
		if element.ident == "complexType" && nested_complex_complex_type {
			...
			if len(element.attribs) != 1 {
				fmt.println(
					"[ERROR] Supposed to be only 1 attribute related to base restriction, found",
					element.attribs,
				)
			} else {
		...
		} else if element.ident == "enumeration" &&
		   len(current_type_def.type_val) > 0 &&
		   parent.ident == "restriction" {
			if current_type_def.type_val == "complexType" && nested_complex_simple_type {
				content := &current_type_def.content.(complex_content)
...
validate_xml :: proc(
	...
	system_node: ^utils.node_tree(^SpaceSystemType) = cast(^utils.node_tree(
		^SpaceSystemType,
	))&xml_handler.system
	xml_handler.tree_eval = xtce_user_tree
	{
		...
		for node_values.push_count > 0 {
			...
			case SPACE_SYSTEM_TYPE:
				{
					...
					for attr in node_it_dst.element.second.attribs {
						if attr.key == "name" {
							system_node.element.base.t_name.t_restriction =
								xs_normalized_string_get_default()
							system_node.element.base.t_name.t_restriction.val = attr.val
						}
						if attr.key == "shortDescription" {
							system_node.element.base.base.t_shortDescription.t_restriction =
								xs_string_get_default()
							system_node.element.base.base.t_shortDescription.t_restriction.val =
								attr.val
						}
					}
				}
			case LONG_DESCRIPTION_TYPE:
				{
					for attr in node_it_dst.element.second.attribs {
						system_node.element.base.base.t_LongDescription.t_restriction =
							xs_string_get_default()
						system_node.element.base.base.t_LongDescription.t_restriction.val =
							attr.val
					}
				}
			case HEADER_TYPE:
			...
			case ARRAY_PARAMETER_TYPE:
				{
					...
					for t.push_count > 0 {
						...
						case DIMENSION_LIST_TYPE:
							{
								array_type.t_DimensionList = LoadDimensionListType(
									n,
									"DimensionList",
								)
							}
			...
			case INTEGER_PARAMETER_TYPE:
				{
					...
					append(
						&system_node.element.t_TelemetryMetaData.t_ParameterTypeSet.t_choice_0,
						int_type,
					)
				}
			case ENUMERATED_PARAMETER_TYPE:
				{
					...
					for t.push_count > 0 {
						...
						case ENUMERATION_CONTEXT_ALARM_LIST_TYPE:
							{
								LoadEnumerationContextAlarmListType(
									node_it_dst,
									&enum_type.t_ContextAlarmList,
								)
							}
...
internal_DepthFirstSearch_Node :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	key: string,
) -> (
	xml.Attribute,
...
LoadFloatingPointNotationType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadChangeAlarmRangesType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadNumericAlarmType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	data: ^NumericAlarmType,
) {
...
LoadParameterRefTypes :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
) -> [dynamic]ParameterRefType {
...
LoadPhysicalAddressSetType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadParameterInstanceRefType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadTimeAssociationUnitType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadTelemetryDataSourceType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadParameterPropertiesType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadErrorDetectCorrectType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadDiscreteLookupListType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadBinaryDataEncodingType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	element: string,
...
LoadParameterRefEntryType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadRestrictionCriteriaType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	element: string,
...
LoadFixedIntegerValueType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	name: string,
		...
		for attr in n.element.second.attribs {
			if attr.key == name {
				...
				case "xs_integer":
					{
						type.t_union =
						cast(xs_integer)strconv.atoi(n.element.second.attribs[0].val)
						found = true
					}
...
LoadSequenceContainerType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadSequenceContainerTypes :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
) -> [dynamic]t_ContainerSetType0 {
...
// TODO: Check for minOccurs
//
LoadContainerSetType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	data: ^ContainerSetType,
) {
...
LoadIntegerDataEncodingType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentIntegerDataType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadValidIntegerRangeSetType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	...
	for stack.push_count > 0 {
		...
		case "xs_boolean":
			{
				type.t_validRangeAppliesToCalibrated.val =
					n.element.second.attribs[0].val == "true" ? true : false
			}
		}
		for it := n.next; it != nil; it = it.right {
			utils.push_stack(&stack, it)
...
LoadArgumentEnumeratedDataType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadEnumeratedArgumentType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentInstanceRefType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentDynamicValueType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentDiscreteLookupListType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentIntegerValueType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	name: string,
...
LoadArgumentDimensionType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	name: string,
...
LoadArgumentDimensionListType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentAssignmentType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentAssignmentListType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentLocationInContainerInBitsType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentMatchCriteriaType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentSequenceEntryType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentFixedValueEntryType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentArgumentRefEntryType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentContainerRefEntryType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadArgumentParameterRefEntryType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadCommandContainerEntryListType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadCommandContainerSetType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadIntegerDataType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	data: ^IntegerDataType,
) {
...
LoadAggregateDataType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	data: ^AggregateDataType,
) {
...
LoadEnumeratedDataType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	data: ^EnumeratedDataType,
) {
	data.base = LoadBaseDataType(node)
...
LoadEnumerationAlarmListType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadEnumerationAlarmType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	data: ^EnumerationAlarmType,
) {
...
LoadBooleanExpressionType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadEnumerationContextAlarmType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
...
LoadEnumerationContextAlarmListType :: proc(
	node: ^utils.node_tree(utils.tuple(string, xml.Element)),
	data: ^EnumerationContextAlarmListType,
) {
	stack: utils.Stack(^utils.node_tree(utils.tuple(string, xml.Element)), 4096)
	utils.push_stack(&stack, node)
...
GetArgumentDecl :: proc(
	system: ^space_system,
	ref: string,
) -> (
	t_ArgumentTypeSetType0,
	t_ParameterTypeSetType0,
) {
...
SearchTypeDeclInSystem :: proc(
	...
	//
	n_depth := strings.count(name, "/")
	n_depth += 1
	path := strings.split_n(name, "/", n_depth, context.temp_allocator)
	defer delete(path, context.temp_allocator)
	if len(path) == 0 || len(path) == 1 {
		delete(path, context.temp_allocator)
		path = make([]string, 1, context.temp_allocator)
